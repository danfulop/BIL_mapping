...
# head(comp.rn, 50)
# m82.idx <- which(comp.rn$genotype=="M82")
# pen.idx <- which(comp.rn$genotype=="PEN")
...
# save(comp.rn, file="comp.rn.Rdata")
load("comp.rn.Rdata")
...
circ.y.labs <- c("Area (µm^2)", "Circularity", "Aspect Ratio", "Roundness", "Solidity") # y-axis labels
# run loop
...
names(circ.list) <- names(circ)[3:7] # name the list elements by their original trait names
...
sym.y.labs <- paste0("Symmetric EFD ", names(sym)[3:11])
# run loop
...
names(sym.list) <- names(sym)[3:11] # name the list elements by their original trait names
...
asym.y.labs <- paste0("Asymmetric EFD ", names(asym)[3:9])
# run loop
...
names(asym.list) <- names(asym)[3:9] # name the list elements by their original trait names
...
FT.y.labs <- "Number of days to first anthesis"
FT.means <- fitMean(x=FT.BIL, y=3, labV=FT.y.labs, i=1, randform="" ) # randform is empty because the FT data is not pseudoreplicated
save(FT.means, file="FT.means.Rdata")
...
names(circ.pred) <- names(circ)[3:7] # name the pred elements by their original trait names
...
names(sym.pred) <- names(sym)[3:11] # name the pred elements by their original trait names
...
names(asym.pred) <- names(asym)[3:9] # name the pred elements by their original trait names
save(asym.pred, file="asym.pred.Rdata") # Save prediction data.frames' list

# Flowering time data
x=FT.BIL; y=3; randform=""
resp <- names(x)[y] # assign the column name of the 'y' index
formula <- as.formula( paste0( get("resp"), " ~ (1|block) ", get("randform") ) )
fit <- lmer(formula, data=x, REML=TRUE)
newdata <- with(x, expand.grid(plant=unique(plant)))
pred <- predict(fit, re.form=NA, type="response", allow.new.levels=TRUE)
predDat <- as.data.frame(cbind(plant=newdata, prediction=pred)) # join the plant labels and predicted responses in a data frame
labs <- x[c('genotype', 'FinBIL', 'plant')] # subset data frame to only have genotype info. and plant ID
labs <- unique(labs) # keep only one row per plant
predDat <- merge(labs, predDat, by="plant")
fit.df <- fit@frame # get the LMM-object's response and factor data frame
fit.df$resid <- residuals(fit, type="response", scaled=FALSE) # calculate residuals
predDat <- cbind(predDat, resid=fit.df$resid)  # append residuals onto predictions
predDat$predPlusResid <- predDat$prediction + predDat$resid
FT.pred <- predDat
# Convert FT.pred into a one element list to suit the mapping functions
FT.pred <- list(FT=FT.pred)
save(FT.pred, file="FT.pred.Rdata")
