...
fitMean <- function(x, y, labV, i, randform) {  # Include as input column index of response values, i.e. y
  ...
  names(tab)[2] <- 'StdErr'; names(tab)[7] <- 'p.value'; names(tab)[3] <- 'lowerCI'; names(tab)[6] <- 'upperCI'
  ...
  fdr <- fdrtool(tab$p.value, statistic="pvalue", plot=FALSE)
...
randform <- "(1|plant)" # specify plant-related random effect term
comp.list <- foreach(i=1:4, .options.multicore=mcoptions) %dopar% { # run loop
  y = i + 4
  fitMean(comp.rn, y, comp.y.labs, i, randform)
}
...
# Function to calculate model-fitted means and generate "swooshPlots" for data pseudo-replicated at the leaflet level
FitMeanPseudoRep <- function(x, y, labV, i) {  # Include as input column index of response values, i.e. y
  resp <- names(x)[y] # assign the column name of the 'y' index
  formula <- as.formula( paste0( get("resp"), " ~ FinBIL + (1|block) + (1|plant/leaflets/leaflet.type)" ) )
  fit <- lmer(formula, data=x, REML=TRUE)
  degfree <- get_ddf_Lb(fit, fixef(fit)) # get Kenward-Roger approximate degrees of freedom
  tab <- as.data.frame( coeftab(fit, df=degfree, p.val=TRUE) ) # save fixed effect table
  rownames(tab)[1] <- "M82" # relabel intercept as M82
